...
from src.utils import sanitize_filename, cleanup_lockfiles
...
class ChzzkRecorder:
    ...
    def _cleanup_old_lockfiles(self):
        ...
        count = cleanup_lockfiles(base_path)
        if count > 0:
    ...
    async def start_recording(self, channel_id: str, live_info: Dict[str, Any]):
        ...
        # 출력 경로 및 파일
        output_path, output_file = self._prepare_output_path(
        ...
        try:
            ...
            # streamlink 명령어 구성
            streamlink_cmd = self._build_streamlink_command(
            ...
            await self._wait_for_stream_end(
            ...
            if process.returncode not in (0, 130):  # 0=정상 종료, 130=SIGINT(Ctrl+C)로 인한 정상 종료로 간주
    ...
    def _prepare_output_path(self, author: str, title: str, stream_start_time: datetime) -> tuple[Path, str]:
        """출력 경로 준비"""
